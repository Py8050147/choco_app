...
const PRODUCTS_CACHE_TTL_SECONDS = 60 * 10; // 10 min
...
export async function POST(request: Request) {
  try {
    ...
    // Invalidate the cache so the next GET reflects the new product
...
export async function GET() {
  try {
    ...
    if (cached) {
      return Response.json(cached); // Upstash auto-deserializes JSON
    }
    ...
    await redis.set(PRODUCTS_CACHE_KEY, allProducts, {
